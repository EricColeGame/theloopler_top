    ...
    <div className="flex items-center justify-between gap-4">
      <Link href={localizeHref("/", locale)} className="flex items-center gap-3">
        <div className="relative h-9 w-9 overflow-hidden rounded-xl border border-border bg-muted shadow-sm">
          <img src="/images/logo.png" alt={`${siteConfig.name} Logo`} className="h-full w-full object-cover" />
        </div>
  ...
  return (
    <div className="group relative cursor-pointer overflow-hidden rounded-2xl border border-border shadow-lg transition-all duration-200">
      <div className="relative aspect-video w-full">
        <img src={videoId ? `https://i.ytimg.com/vi/${videoId}/maxresdefault.jpg` : "/images/hero.webp"} alt={`${siteConfig.name} Official Trailer`} className="size-full object-cover transition-all duration-200 group-hover:brightness-80" onError={(e) => { (e.target as HTMLImageElement).src = "/images/hero.webp"; }} />
      </div>
